...
在实际应用中ResNet在对特征图进行降维时，每个卷积构建单元的输入和输出维度不同，此时使用恒等连接会使两个维度不一样的特征向量相加，造成错误。则应使用投影连接，将单个卷积构建单元中残差分支的维度与卷积核主体的维度相统一。但此时，统一特征维度所使用的卷积操作为密集连接，一定程度上会增加模型的大小。相对来说，恒等连接不会增加模型的参数和计算的复杂度。因此在卷积构建单元无降维操作时，直接使用恒等连接是更优的选择。

本模型在使用ResNet时，101层的网络中存在5次降维操作，即模型的大部分残差连接都由恒等连接构建而成。

\subparagraph{零填充} 
零填充也是在残差分支与卷积分支维度不同时的一种统一维度的方法。它直接在增加的维度上填充0来匹配二者的输出，但它的效果并不如投影连接好，仅作为在运算性能受限的情况下提供的一种构建选项。
...
因此，ResNet~\cite{He2016DeepRL}提出了{\itshape bottleneck}设计来替换原来的building block。如图~\ref{fig:resnet_bottleneck}，对于每个残差函数$\mathcal{F}$，我们使用了三个卷积层来代替原有的两层，分别是$1 \times 1$，$3 \times 3$和$1 \times 1$卷积，其中$1 \times 1$层负责减小然后增加（恢复）维度，使$3 \times 3$层作为bottleneck，可以具有较小的输入和输出，大大降低了模型的时间复杂度。
